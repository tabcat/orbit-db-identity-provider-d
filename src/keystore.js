...
const unmarshalPrivKey = (pk) => new Promise((resolve, reject) => {
...
class Keystore {
  ...
  async open () {
    if (this.mem === undefined) this._mem = new Map()
    if (this._store) {
  ...
  async deriveKey (id, pw) {
    if (!id) {
      throw new Error('id needed to create a key')
    }
    console.log(id)
    if (!pw) {
      throw new Error('pw needed to create a key')
    }
    if (this._store.status && this._store.status !== 'open') {
      return Promise.resolve(null)
    }
    ...
    let privateKey
    do {
      if (i > 10) throw new Error('unabled to create valid private key')
      const pbkdf2 = await crypto.pbkdf2(pw, pw, 10000 + i, 32, 'sha2-256')
      privateKey = Buffer.from(pbkdf2, 'base64')
      i++
    } while (!secp256k1.privateKeyVerify(privateKey))

    const keys = await unmarshalPrivKey(privateKey)

    this._mem.set(id, keys.marshal())
  ...
  async getKey (id) {
    ...
    if (this._mem.has(id)) return await unmarshalPrivKey(this._mem.get(id))
    ...
    return unmarshalPrivKey(Buffer.from(deserializedKey.privateKey, 'hex'))
  }
